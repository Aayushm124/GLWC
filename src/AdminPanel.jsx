...
const CAT_LABELS = { mousepads: 'Mouse Pads', home: 'Home Decor', led: 'LED Products' };
    ...
    <div onClick={() => fileRef.current.click()}
      ...
      {value ? (
        ...
        <div style={{ textAlign: 'center', pointerEvents: 'none' }}>
          ...
          <div style={{ fontSize: '0.82rem', color: 'rgba(255,181,21,0.98)', fontWeight: 600, marginTop: 8 }}>{uploading ? 'Uploading...' : 'Click or drag & drop'}</div>
...
function ProductForm({ initial, onSave, onCancel }) {
  const empty = { name: '', cat: 'mousepads', image: '', image2: '', image3: '', price: '', old: '', desc: '', meesho: '', amazon: '', rating: '', reviews: '', badges: [], colors: ['#daa532'] };
  ...
  return (
    <div id="edit-form" style={{ background: 'rgba(16,14,24,0.98)', border: '1px solid rgba(218,165,50,0.2)', borderRadius: 20, padding: '2rem', backdropFilter: 'blur(20px)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 2rem' }}>
        ...
        <div>
          ...
          <Field label="Total Reviews">
            <input style={INPUT} type="number" placeholder="e.g. 128" value={form.reviews || ''} onChange={e => set('reviews', e.target.value)}
...
{CATEGORIES.map(c => <option key={c} value={c} style={{ background: '#0e0c18' }}>{CAT_LABELS[c]}</option>)}            </select>
          </Field>
        </div>
        ...
        <div>
          <Field label="Product Name">
            <input style={{ ...INPUT, borderColor: errors.name ? '#ef4444' : 'rgba(255,255,255,0.1)' }} value={form.name} placeholder="e.g. GLCW Anime Mouse Pad"
          ...
          <Field label="Rating (Out of 5)">
            <input style={INPUT} type="number" step="0.1" max="5" min="0" placeholder="e.g. 4.5" value={form.rating || ''}
              onChange={e => set('rating', e.target.value)}
              onFocus={e => e.target.style.borderColor = 'rgba(218,165,50,0.5)'}
              onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
          </Field>
          <Field label="Short Description">
            <input style={INPUT} value={form.desc} placeholder="e.g. 60x30cm, Non-Slip Rubber Base"
          ...
          {(form.name || form.image) && (
            <div style={{ padding: '0.75rem 1rem', borderRadius: 12, background: 'rgba(218,165,50,0.05)', border: '1px solid rgba(218,165,50,0.12)', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: '0.88rem', color: '#f2ede4' }}>{form.name || 'Product Name'}</div>
  ...
  return (
    <div style={{ background: 'rgba(16,14,24,0.98)', border: '1px solid rgba(218,165,50,0.15)', borderRadius: 16, padding: '1.5rem', backdropFilter: 'blur(20px)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div>
          <Field label="Product Name">
            <input style={{ ...INPUT, borderColor: errors.name ? '#ef4444' : 'rgba(255,255,255,0.1)' }} value={form.name} placeholder="e.g. Anime Mouse Pad"
              onChange={e => set('name', e.target.value)}
              onFocus={e => e.target.style.borderColor = 'rgba(218,165,50,0.5)'}
              onBlur={e => e.target.style.borderColor = errors.name ? '#ef4444' : 'rgba(255,255,255,0.1)'} />
            {errors.name && <span style={{ fontSize: '0.7rem', color: '#ef4444' }}>{errors.name}</span>}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#04040a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 380, padding: '2.5rem', background: 'rgba(16,14,24,0.98)', border: `1px solid ${locked ? 'rgba(239,68,68,0.3)' : 'rgba(218,165,50,0.2)'}`, borderRadius: 20, textAlign: 'center', boxShadow: '0 40px 80px rgba(0,0,0,0.5)' }}>
        <div style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: '1.8rem', background: 'linear-gradient(120deg, #daa532, #f0c060)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: 6 }}>GLCW</div>
        ...
        {locked ? (
          ...
          <div>
            ...
            {error && <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, color: '#ef4444', fontSize: '0.78rem', marginBottom: '0.75rem', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '0.5rem' }}><Icons.X size={13} color="#ef4444" /> {error}</div>}
            {attemptsLeft < 5 && !error && <div style={{ fontSize: '0.72rem', color: '#666', marginBottom: '0.75rem' }}>{attemptsLeft} attempt{attemptsLeft === 1 ? '' : 's'} remaining before lockout</div>}
...
function CarouselManager() {
  ...
  return (
    <div>
      {successMsg && <div style={{ position: 'fixed', top: 80, right: 24, zIndex: 500, background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.4)', borderRadius: 12, padding: '0.75rem 1.25rem', color: '#22c55e', fontSize: '0.85rem', fontWeight: 600, backdropFilter: 'blur(20px)' }}>{successMsg}</div>}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: 6 }}>
          {CAROUSEL_TABS.map(t => (
            <button key={t.key} onClick={() => { setActiveTab(t.key); setShowForm(false); setEditingItem(null); }} style={{ padding: '0.45rem 1.1rem', borderRadius: 10, cursor: 'pointer', transition: 'all 0.2s', fontSize: '0.82rem', fontWeight: activeTab === t.key ? 700 : 400, border: `1px solid ${activeTab === t.key ? 'rgba(218,165,50,0.5)' : 'rgba(255,255,255,0.08)'}`, background: activeTab === t.key ? 'rgba(255,255,255,0.12)' : 'transparent', color: activeTab === t.key ? '#daa532' : '#555' }}>{t.label} <span style={{ fontSize: '0.7rem', opacity: 0.6 }}>({(carousel[t.key] || []).length})</span></button>
          ))}
        </div>
        {!showForm && !editingItem && (
          <button onClick={() => { setShowForm(true); setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100); }} style={{ padding: '0.5rem 1.25rem', borderRadius: 10, background: 'linear-gradient(135deg, #daa532, #f0c060)', border: 'none', color: '#000', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer' }}>+ Add Item</button>
        )}
      </div>
      {showForm && <div ref={formRef} style={{ marginBottom: '1.5rem' }}><div style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '0.95rem', color: 'rgba(218,165,50,0.6)', marginBottom: '0.75rem' }}>Add to {CAROUSEL_TABS.find(t => t.key === activeTab)?.label}</div><CarouselForm onSave={handleAdd} onCancel={() => setShowForm(false)} /></div>}
      {editingItem && <div ref={formRef} style={{ marginBottom: '1.5rem' }}><div style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '0.95rem', color: 'rgba(218,165,50,0.6)', marginBottom: '0.75rem' }}>Editing: {editingItem.name}</div><CarouselForm initial={editingItem} onSave={handleUpdate} onCancel={() => setEditingItem(null)} /></div>}
      {items.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#333', fontSize: '0.85rem' }}>No items yet. Click "+ Add Item" to get started.</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {items.map(item => (
            <div key={item.id} style={{ background: 'rgba(16,14,24,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ height: 120, background: 'rgba(218,165,50,0.04)', overflow: 'hidden' }}>
                {item.image ? <img src={item.image} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icons.Package size={32} color="rgba(218,165,50,0.4)" /></div>}
              </div>
              <div style={{ padding: '0.75rem' }}>
                <div style={{ fontWeight: 600, fontSize: '0.82rem', marginBottom: 3, color: '#f2ede4', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</div>
                <div style={{ fontFamily: 'Syne', fontWeight: 700, color: '#daa532', fontSize: '0.9rem', marginBottom: '0.6rem' }}>₹{item.price}</div>
                <div style={{ display: 'flex', gap: 5, marginTop: 8 }}>
                  <button onClick={() => { setEditingItem(item); setShowForm(false); setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100); }} style={{ flex: 1, padding: '4px 0', borderRadius: 7, background: 'rgba(218,165,50,0.1)', border: '1px solid rgba(218,165,50,0.25)', color: '#daa532', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>Edit</button>
                  {deleteConfirm === item.id ? (
                    ...
                    <button onClick={() => setDeleteConfirm(item.id)} style={{ flex: 1, padding: '4px 0', borderRadius: 7, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#ef4444', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>Delete</button>
                  )}
                </div>
              </div>
            </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        ...
        {!showForm && !editingProduct && (
          <button onClick={() => { setShowForm(true); setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100); }} style={{ padding: '0.6rem 1.5rem', borderRadius: 10, background: 'linear-gradient(135deg, #daa532, #f0c060)', border: 'none', color: '#000', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}>+ Add New Product</button>
        )}
      </div>
      {showForm && <div ref={formRef} style={{ marginBottom: '2rem' }}><div style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '1rem', color: 'rgba(218,165,50,0.6)', marginBottom: '1rem' }}>Add New Product</div><ProductForm onSave={handleAdd} onCancel={() => setShowForm(false)} /></div>}
      {editingProduct && <div ref={formRef} style={{ marginBottom: '2rem' }}><div style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '1rem', color: 'rgba(218,165,50,0.6)', marginBottom: '1rem' }}>Editing: {editingProduct.name}</div><ProductForm initial={editingProduct} onSave={handleUpdate} onCancel={() => setEditingProduct(null)} /></div>}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {products.map(p => (
          <div key={p.id} style={{ background: 'rgba(16,14,24,0.8)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ width: 56, height: 56, borderRadius: 10, overflow: 'hidden', flexShrink: 0, background: 'rgba(218,165,50,0.08)' }}>
              {p.image ? <img src={p.image} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icons.Package size={26} color="rgba(218,165,50,0.4)" /></div>}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontWeight: 600, fontSize: '0.92rem', color: '#f2ede4', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 300 }}>{p.name}</span>
              ...
              <div style={{ fontSize: '0.72rem', color: '#555', marginTop: 2 }}>{CAT_LABELS[p.cat] || p.cat} · {p.desc}</div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
              <button onClick={() => { setEditingProduct(p); setShowForm(false); setTimeout(() => formRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100); }} style={{ padding: '0.4rem 0.9rem', borderRadius: 8, cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, background: 'rgba(218,165,50,0.1)', border: '1px solid rgba(218,165,50,0.25)', color: '#daa532' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Icons.Edit size={13} color="#daa532" />Edit</span>
              </button>
              {deleteConfirm === p.id ? (
                <div style={{ display: 'flex', gap: 4 }}>
                  <button onClick={() => handleDelete(p.id)} style={{ padding: '0.4rem 0.75rem', borderRadius: 8, cursor: 'pointer', background: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.4)', color: '#ef4444', fontSize: '0.72rem', fontWeight: 700 }}>Yes</button>
                ...
                <button onClick={() => setDeleteConfirm(p.id)} style={{ padding: '0.4rem 0.9rem', borderRadius: 8, cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#ef4444' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Icons.Trash size={13} color="#ef4444" />Delete</span>
                </button>
              )}
            </div>
          </div>
    ...
    <div style={{ minHeight: '100vh', background: '#04040a', fontFamily: 'Instrument Sans, sans-serif' }}>
      <div style={{ background: 'rgba(10,10,18,0.95)', borderBottom: '1px solid rgba(218,165,50,0.15)', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 200, backdropFilter: 'blur(20px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: '1.4rem', background: 'linear-gradient(120deg, #daa532, #f0c060)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>GLCW</div>
          <span style={{ fontSize: '0.65rem', padding: '2px 10px', borderRadius: 999, background: 'rgba(218,165,50,0.12)', color: '#daa532', border: '1px solid rgba(255,174,0,0.25)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Admin Panel</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <a href="/" style={{ fontSize: '0.8rem', color: '#666', textDecoration: 'none', padding: '0.4rem 1rem', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8 }}>← View Site</a>
          <button onClick={() => { logout(); setLoggedIn(false); }} style={{ fontSize: '0.8rem', color: '#666', background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '0.4rem 1rem', cursor: 'pointer' }}>Logout</button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: '2rem', background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 4, width: 'fit-content' }}>
          {[{ key: 'products', label: '📦 Products', count: products.length }, { key: 'carousel', label: '🎠 Carousel', count: totalCarousel }].map(t => (
            <button key={t.key} onClick={() => setMainTab(t.key)} style={{ padding: '0.55rem 1.5rem', borderRadius: 9, cursor: 'pointer', transition: 'all 0.2s', fontSize: '0.85rem', fontWeight: mainTab === t.key ? 700 : 400, background: mainTab === t.key ? 'rgba(255,255,255,0.15)' : 'transparent', border: `1px solid ${mainTab === t.key ? 'rgba(218,165,50,0.4)' : 'transparent'}`, color: mainTab === t.key ? '#daa532' : '#555' }}>
